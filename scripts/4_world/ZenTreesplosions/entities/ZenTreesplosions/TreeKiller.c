class TreeKiller
{
	static void KillTree(vector pos, string ammo)
	{
		...
		TreesplosionType tt = GetZenTreesplosionsConfig().GetTreesplosionType(ammo);
		...
		g_Game.GetObjectsAtPosition3D(pos, tt.Radius, objectsNearBomb, null);
		...
		for (int x = 0; x < objectsNearBomb.Count(); x++)
		{
			...
			{
				// Kill tree
				obj.DecreaseHealth("", "", obj.GetMaxHealth("",""), obj.CanBeAutoDeleted());
